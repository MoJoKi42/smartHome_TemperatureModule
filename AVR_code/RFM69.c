...
// **********************************************************************************
// https://github.com/nayem-cosmic/RFM69-Library-AVR/tree/master
// **********************************************************************************
...
void rfm69_init(uint16_t freqBand, uint8_t nodeID, uint8_t networkID)
{
    ...
    SS_DDR &= ~(1<<SS_PIN);
    SS_PORT &= ~(1<<SS_PIN);     // setting slave select low (prepare)
    ...
    EICRA |= (1<<ISCn1)|(1<<ISCn0); // setting INTn rising. details datasheet p91. must change with interrupt pin.
